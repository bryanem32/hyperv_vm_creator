...
"@
        ...
        if ($chkFULLUPDATE.Checked) {
            $lines += @(
                ':: --- Schedule full Windows + .NET Updates for first logon ---'
                'echo [%date% %time%] Scheduling full Windows + .NET Update on first login... >> %LOGFILE%'
                ...
                'echo powershell -ExecutionPolicy Bypass -NoProfile -Command "try { Install-PackageProvider -Name NuGet -Force; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Install-Module PSWindowsUpdate -Force -Scope AllUsers; Import-Module PSWindowsUpdate; Add-WUServiceManager -MicrosoftUpdate -Confirm:$false -ErrorAction SilentlyContinue; Write-Output (''['' + (Get-Date) + ''] Checking for Windows + .NET updates...''); Get-WindowsUpdate -MicrosoftUpdate -AcceptAll -Install -IgnoreReboot | Out-File -FilePath C:\Windows\Temp\WUOutput.log -Encoding UTF8; Write-Output (''['' + (Get-Date) + ''] Updates completed successfully.''); } catch { Write-Output (''['' + (Get-Date) + ''] Update process failed: '' + $_.Exception.Message); }" >> C:\Windows\Temp\RunUpdates.cmd'
