...
$win11pro = $osversion -match 'Windows 11 (Pro|Enterprise)'
...
function Get-GpuPProviders {
    ...
    foreach ($gpu in $gpus) {
        ...
        # Skip GPUs that contain "AI" in the name (case-insensitive)
        if ($friendlyName -match '(?i)\bAI\b') {
            continue
        }
    ...
    # Add "Remove GPU-P Adapter" as first item
...
function Copy-Drivers {
    ...
    # If target exists, delete it forcibly (ownership + ACL fix only)
    if ($ForceDelete -and (Test-Path $target)) {
        Write-Log ("[$($VMName)] Target exists, deleting $target first")
        try {
            # Take ownership ONLY if NTFS (silently)
            ...
            Write-Log ("[$($VMName)] ERROR deleting existing directory: $($_.Exception.Message)")
        }
    }

    # Ensure directory exists (recreate)
    if (-not (Test-Path $target)) {
        Write-Log ("[$($VMName)] Creating directory $target")
...
function Update-GPUDrivers {
    # Populate GpuPList
    if ($win11pro -eq $true) {
        $providerObj = $GpuPList | Where-Object {
            $_.Friendly -eq $GpuSelector.SelectedItem
        }
        if (-not $providerObj) {
            Write-Log "No GPU selected"
            return
        }
    ...
    foreach ($item in $VMCheckboxes) {
        if ($item.Checked) {
            ...
            try {
                ...
                # VM GPU Partition Adapter Reset
                Start-Sleep -Seconds 3
                Remove-VMGpuPartitionAdapter -VMName $VMName -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 2
                
                # Remove GPU-P Adapter only
                if ($providerObj.Provider -eq $null -and $win11pro -eq $true) {
                    Write-Log ("[$($VMName)] Removing GPU-P Adapter only")
                }
                else { 
                    try {
                        # GPU-P adapter create
                        if ($win11pro -eq $true) {
                            # Windows 11 Pro|Enterprise only with Instance Path
                            Write-Log ("[$($VMName)] Creating new GPU Partition Adapter with $($providerObj.Friendly)...")
                            Add-VMGpuPartitionAdapter -VMName $VMName -InstancePath $providerObj.Name -ErrorAction SilentlyContinue
                        } else {
                            # Windows 10 default
                            Write-Log ("[$($VMName)] Creating new GPU Partition Adapter...")
                            Add-VMGpuPartitionAdapter -VMName $VMName
                        }

                        # Adapter Settings for Memory
                        ...
                        Write-Log ("[$($VMName)] GPU Partition Adapter created and configured.")
                    } catch {
                ...
                Copy-Drivers -VMName $VMName -MountLetter $mountLetter -Source $HostDriverStore -Destination $VMDriverStore -ForceDelete

                # Determine if Windows 11 Pro or not
                if ($win11pro -eq $true) {
                    
                    #Windows 11 Pro|Enterprise
                    Write-Log ("[$($VMName)] Using GPU Vendor $($gpuVendor). Copying necessary files...")
                    switch ($gpuVendor) {
                        "NVIDIA" { Copy-Drivers -VMName $VMName -MountLetter $mountLetter -Source "C:\Windows\System32" -Destination "Windows\System32" -FileMask "nv*" }
                        "AMD"    { Copy-Drivers -VMName $VMName -MountLetter $mountLetter -Source "C:\Windows\System32" -Destination "Windows\System32" -FileMask "amdkmd*" }
                        "Intel"  { Copy-Drivers -VMName $VMName -MountLetter $mountLetter -Source "C:\Windows\System32" -Destination "Windows\System32" -FileMask "igfx*" }
                    }
                } else {
                    
                    # Windows 10 Default - Extra step for NVIDIA GPU if host has any
                    if ($HasNvidiaGPU) {
...
$Form.Text = "Hyper-V VM GPU Updater v28"
...
# GPU Selector if Windows 11 Pro or Enterprise
if ($win11pro -eq $true) {
    # Select GPU Label
    $label = New-Object System.Windows.Forms.Label
    $label.Text = "Select GPU-P Enabled GPU:"
    $label.AutoSize = $true
    $label.Location = New-Object System.Drawing.Point(320,10)
    $Form.Controls.Add($label)
    
    # GPU Selector Dropbox
    $GpuSelector = New-Object System.Windows.Forms.ComboBox
    $GpuSelector.Location = "350,40"
    $GpuSelector.Width = 300
    $Form.Controls.Add($GpuSelector)
}

$GpuPList = Get-GpuPProviders
$GpuPList | ForEach-Object { $GpuSelector.Items.Add($_.Friendly) | Out-Null }
...
$StartVMCheckbox.Location = New-Object System.Drawing.Point(20,[int]($y + 30))
...
$LogBox.Location = New-Object System.Drawing.Point(20,[int]($y + 60))
...
$UpdateButton.Location = New-Object System.Drawing.Point(160,540)
$UpdateButton.Size = New-Object System.Drawing.Size(160,30)
...
$ExitButton.Location = New-Object System.Drawing.Point(380,540)
...
# LogBox Header Text
$LogBox.AppendText("Hyper-V VM GPU Updater v28 `r`n")
